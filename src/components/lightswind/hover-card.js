...
const HoverCardContent = React.forwardRef(({ className, align = "center", sideOffset = 4, ...props }, ref) => {
    ...
    return (_jsx(AnimatePresence, { children: open && ( // Conditionally render the motion.div based on `open` state
        _jsx(motion.div, { ref: ref, initial: { opacity: 0, scale: 0.95, y: -5 }, animate: { opacity: 1, scale: 1, y: 0 }, exit: { opacity: 0, scale: 0.95, y: -5 }, transition: { duration: 0.2, ease: "easeOut" }, className: cn(`absolute z-50 w-64 rounded-xl border border-white/10 bg-[#111827]/95 p-4 text-slate-100 shadow-[0_20px_45px_rgba(0,0,0,0.35)] backdrop-blur-md`, className), style: {
